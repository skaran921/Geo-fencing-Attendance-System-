...
<div class="table-responsive table-striped table-hoverable" style="padding-bottom: 20px;">
    ...
    <table id="myTable" class=" table table-bordered">
        ...
        <tbody>
            ...
            include "./db.php";
            $sql = "SELECT *,messages.id As message_id,admin.name AS admin_name,admin.image As admin_image,employee.emp_name,employee.image As emp_image
         FROM messages
           left join admin ON messages.admin_id=admin.id 
           left join employee ON messages.emp_id=employee.emp_id
           WHERE messages.admin_id='$admin_id' AND messages.status='Inactive' ORDER BY messages.u_id DESC";
            ...
            ?>
                <tr>
                    ...
                    <td>
                        ...
                        ?>
                            <img src="./employee/<?php echo $row['emp_image']; ?>" class="img-fluid" style="width:60px;height:50px;border-radius:50%" alt="img">
                        <?php
                            echo $row["emp_name"];
                        } else {
                        ?>
                            <img src="./images/<?php echo $row['admin_image']; ?>" class="img-fluid" style="width:60px;height:50px;border-radius:50%" alt="img">
                        <?php
                            echo $row["admin_name"];
                        }
                        ?>
                        ...
                        <form action="" method="post">
                            <div class="dropdown">
                                ...
                                <div class="dropdown-content">
                                    <a href="javascript:void(0)" onclick="alertify.confirm('<h4>Alert</h4>','<h1>Are You Sure?</h1>',function(){removeDeleteMessage(<?php echo $row['message_id'] ?>)},function(){ alertify.warning('Cancel')});">
